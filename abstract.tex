...
\title{Higher-order \textsf{fork}, modaly (Extended Abstract)}
...
\textsf{OCaml} even though it only supports single-shot
...
\textsf{OCaml} has no static effect system, the type signatures are
straightforward.

We present implementations of a scheduler for cooperative concurrency with
higher-order fork in \textsf{Koka}, \textsf{Frank}, \textsf{Effekt} and
\textsf{\textsc{Metl}}. Then, we overview a new way to express higher-order
effects with modal effect types and use a language based on it to implement such
a scheduler.
...
\textsf{fork} that takes a thunk as input. To demonstrate compositionality of
the type-and-effect systems, schedulers will use a parametric \textsf{queue}
...
allowing users to omit effect variables.
...
\textsf{Effekt}~\cite{effekt2020, effekt2022} takes another path to avoid
cluttering type signatures with effect variables. 
...
\end{lstlisting}
Another difference with the other implementations is the absence of masking. The
semantics wraps the operands of higher-order effects with the same masks as the
yielding computation. Thus, there is still a mask in the first call to
\textsf{schedule}.
\begin{lstlisting}[style=metl]
  val full_schedule : [](<co>{unit} -> unit)
  let full_schedule m =
    queue { schedule { mask<state> (m!) } }
\end{lstlisting}
