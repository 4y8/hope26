...
\textsf{Koka} has a traditional type-and-effect system where each arrow type
holds a scoped row~\cite{scopedrows2005} of effects the function can perform. We
rely on effect polymorphism to store and yield effectful functions while
pleasing the type system, modeling concurrency with this effect.
...
here. The concurrency effect has signature:
\begin{lstlisting}[style=Frank]
  interface Co = fork : {[Co]Unit} -> Unit
               | yield : Unit
\end{lstlisting}
where \lstinline[style=Frank]!{[Co]Unit}!  corresponds to a thunk with effect
\lstinline[style=Frank]{Co}. By defining the type \lstinline[style=Frank]{Proc}
to be \lstinline[style=Frank]!proc {[Queue Proc]Unit}!, we can implement a
scheduler with the following variable-free type.
\begin{lstlisting}[style=Frank]
...
modality to the continuation. This extra annotation brings extra burden compared
to \textsf{Koka}.
