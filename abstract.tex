...
\begin{abstract}
  ...
  handlers while guaranteeing safety, one can use a static type-and-effect
  system. Implementing cooperative concurrency with a higher-order fork can show
  shortcomings of such a system.

  We review implementations of cooperative concurrency in existing
  languages. This motivates the development of a new language based on modal
  effect types which we also use to implement this example.
\end{abstract}
\maketitle
...
\textsf{fork} that takes a thunk as input, in several languages. To demonstrate
compositionality of the type-and-effect systems, schedulers will use a
parametric \textsf{queue} effect with two operations (\textsf{enqueue} and
\textsf{dequeue}) to store thunks.
